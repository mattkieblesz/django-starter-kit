...
class Importer(object):
    ...
    def load_module(self, fullname):
        ...
        try:
            ...
            from <% project_name %>.utils.settings import reraise_as
            msg = str(e)
            if msg:
    ...
    def _load_module(self, fullname):
        ...
        install_plugin_apps('<% project_name %>.apps', settings_mod)
...
def add_settings(mod, settings):
    '''
    Adds all settings that are part of ``mod`` to the global settings object. Special cases ``EXTRA_APPS`` to append
    the specified applications to the list of ``INSTALLED_APPS``.
    '''
    ...
    for setting in dir(mod):
        ...
        if setting in ('INSTALLED_APPS', 'TEMPLATE_DIRS') and isinstance(setting_value, str):
