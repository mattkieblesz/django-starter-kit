from <% project_name %>.exceptions import InvalidConfiguration
from <% project_name %>.utils import warnings


class Version(tuple):
    def __str__(self):
        return '.'.join(map(bytes, self))
...
def make_upgrade_message(service, modality, version, hosts):
    return '{service} {modality} be upgraded to {version} on {hosts}.'.format(
        hosts=','.join(map(bytes, summarize(hosts.keys(), 2))),
        modality=modality,
        service=service,
        version=version,
    )
